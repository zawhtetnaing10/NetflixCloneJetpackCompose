package com.zg.netflixloginscreenjetpackcompose.ui.theme

import androidx.compose.ui.graphics.Color

val Black = Color(red = 0, green = 0, blue = 0)
val NetflixRedPrimary = Color(red = 209, green = 47, blue = 38)
val NetflixRed = Color(red = 83, green = 14, blue = 13)
val NetflixGrey = Color(red = 50, green = 50, blue = 50)
val White = Color(red = 255, green = 255, blue = 255)
val HintGrey = Color(red = 170, green = 170, blue = 170)

val HomeScreenGradient = Color(red = 41, green = 79, blue = 83)
val FeaturedMovieGradient = Black.copy(alpha = 0.5f)
val ContinueWatchingBackground = Color(red = 22, green = 22, blue = 22)
val BottomNavigationBackground = Color(red = 30, green = 30, blue = 30)
val BottomNavigationUnSelectedColor = Color(red = 131, green = 131, blue = 131)
val MovieDetailsIcon = Color(red = 168, green = 168, blue = 168)
val CastTextColor = Color(red = 124, green = 124, blue = 124)